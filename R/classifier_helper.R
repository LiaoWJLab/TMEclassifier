...
#'
#' @examples
varsfilter<-function(eset, vars = "row", remove_prop = 0.2){

  if(vars=="row"){
    vars <- apply(eset, 1, sd)
  }else{
    vars <- apply(eset, 2, sd)
  }
  eset<- eset[vars > quantile(vars, remove_prop), ]

  cat(crayon::green(paste0(remove_prop*100,"% of genes with low sd were removed \n")))
  return(eset)
}
...
#' @examples
#'
surv_cluster<-function(input_pdata,
                       target_group,
                       ID              = "ID",
                       levels          = c("High","Low"),
